...
# load data  
```{r}
# geo geo accession of each group
amd_geo_accession <- GSE135092[[1]] %>% pData() %>% 
    dplyr::filter(`tissue:ch1` == "RPE, Macula", `amd_status:ch1` == "AMD") %>% 
    pull(geo_accession)

control_geo_accession <- GSE135092[[1]] %>% pData() %>% 
...
for(i in c(amd_geo_accession, control_geo_accession)){
    reads = read.table(file = list.files(path = "f:/git/Data/AMD/bulk-normal-amd/", 
                                                 pattern = i, 
                                                 full.names = T), 
                               sep = "\t", 
                               header = T, 
...
# check the equality of rownames
a <- c()
...
write.table(x = reads_table, 
            file = "f:/git/R-codes/Retina/bulk-amd-normal/reads_table_amd_ctrl.csv", 
            row.names = T, 
            col.names = T, 
            quote = F)
...
keep <- rowSums(cpm_reads > filter_thresh) > length(c(y$samples$group)) * 0.2
y_keep <- y[keep, ]
```

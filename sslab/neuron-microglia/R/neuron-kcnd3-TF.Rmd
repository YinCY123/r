...
```{r}
sce.zeisel <- readRDS("F:/git/Bioconductor/workflow/Single-Cell/scRNA-seq-online/data/sce.zeisel.rds")
sce.zeisel %>% colData() %>% .[, "level1class"] %>% table()

sce.zeisel.ca1 <- sce.zeisel[, sce.zeisel$level1class == "pyramidal CA1"]

keep <- assay(sce.zeisel.ca1, "counts") %>% 
  apply(1, function(x){x > 0}) %>% 
  t() %>% 
  apply(1, function(x){(sum(x)/939) > 0.10}) 

genes_expressed_in_ca1 <- sce.zeisel.ca1[keep, ] %>% rownames()

expressed_tf_in_ca1 <- tf %>% 
  filter(SYMBOL %in% genes_expressed_in_ca1)
```
...
dist_to_pdgfrb <- shortest_paths(graph = ig, 
                                 ...
                                 to = V(ig)$name %in% expressed_tf_in_ca1$ENTREZID, 
                                 mode = "out", 
                                 output = "epath")$epath
...
N <- tibble(
  ...
  mutate(
    ...
    ncol = "grey", 
    ncol = ifelse(names_to_display[ids] %in% expressed_tf_in_ca1$SYMBOL, "gold", ncol)
  )
...
ggsave("../res/tf_signale-20210401.pdf", device = cairo_pdf)
```

already got primers
```{r}
expressed_tf_in_ca1[expressed_tf_in_ca1$SYMBOL %in% c("Stat1", "Sp1", "Hes1", 
                                                      "Tfe3", "Bhlhe41"), ]
```

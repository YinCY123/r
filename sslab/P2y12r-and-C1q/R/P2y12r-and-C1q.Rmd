...
intersects_pvn_top100_DEG_A3_L3_DGE_df <- select(x = org.Mm.eg.db, 
       keys = intersect(pvn_top_100_GO_DEG, intersects_A3_L3), 
       keytype = "SYMBOL", 
       column = c("ENTREZID", "SYMBOL"))

A3_L3_top100_DEG_pathways <- keggLink(target = "pathway", 
         source = paste("mmu:", intersects_pvn_top100_DEG_A3_L3_DGE_df$ENTREZID, sep = "")) %>% 
    unique() %>% 
    gsub("path:", replacement = "", x = .)
```

```{r}
download.kegg(pathway.id = A3_L3_top100_DEG_pathways %>% gsub("mmu", "", .), 
              species = "mmu", 
              kegg.dir = "../res/P2ry12/A3_L3_top100_pathways/")
```
...
```{r}
pathways <- list.files(path = "../res/P2ry12/A3_L3_top100_pathways/", 
                       pattern = ".xml$", 
                       full.names = T)
...
for(i in seq_along(pathways)){
    pathways_list[[i]] <- parseKGML2Graph(file = pathways[i], 
                                          genesOnly = F, 
                                          expandGenes = F)
}

pathways_merged <- mergeKEGGgraphs(list = pathways_list, edgemode = "directed")

...
# using data in 20201204 folder
files <- list.files(path = "../data/20201204/", full.names = T)
brain_region <- rep(c("BS", "CTX", "PVN"), each = 4)
treatments <- rep(c("3L", "3A", "14L", "14A"), time = 3)
...
#  filter(!(nchar(symbol) == 0)) %>% 
  ...
  arrange(group) %>% 
  write.csv(file = "../res/P2ry12/number_of_unique_genes_per_group-20200104.csv", 
              row.names = F, 
              quote = F)
```
...
```{r}
df %>% 
    write.csv(file = "../res/P2ry12/genes_per_group-20200104.csv", row.names = F, quote = F)
```

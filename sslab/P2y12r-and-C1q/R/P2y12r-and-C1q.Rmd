...
lty <- c(rep(1, 4), 5, 1, 3, 1, 5, 3, 5, 1, 3, 1)
names(lty) <- all_path_df$subtype %>% unique() %>% na.omit() %>% as.character()
...
all_path_df %>% filter(is.na(from)|is.na(to))
```
...
all_path_gene_graph <- graph_from_data_frame(d = all_path_gene_df, directed = T)
p2ry12_order1 <- make_ego_graph(all_path_gene_graph, order = 1, nodes = "P2ry12", mode = "out")[[1]]
p2ry12_order2 <- make_ego_graph(all_path_gene_graph, order = 2, mode = "out", nodes = "P2ry12")[[1]]
p2ry12_order3 <- make_ego_graph(all_path_gene_graph, order = 3, mode = "out", nodes = "P2ry12")[[1]]
p2ry12_order4 <- make_ego_graph(all_path_gene_graph, order = 4, mode = "out", nodes = "P2ry12")[[1]]
p2ry12_order5 <- make_ego_graph(all_path_gene_graph, order = 5, mode = "out", nodes = "P2ry12")[[1]]
p2ry12_order6 <- make_ego_graph(all_path_gene_graph, order = 6, mode = "out", nodes = "P2ry12")[[1]]

genes_in_order0 <- "P2ry12"
genes_in_order1 <- V(p2ry12_order1)[1:length(V(p2ry12_order1))] %>% attr("name") %>% unique()
genes_in_order2 <- V(p2ry12_order2)[1:length(V(p2ry12_order2))] %>% attr("name") %>% unique()
genes_in_order3 <- V(p2ry12_order3)[1:length(V(p2ry12_order3))] %>% attr("name") %>% unique()
genes_in_order4 <- V(p2ry12_order4)[1:length(V(p2ry12_order4))] %>% attr("name") %>% unique()
genes_in_order5 <- V(p2ry12_order5)[1:length(V(p2ry12_order5))] %>% attr("name") %>% unique()
genes_in_order6 <- V(p2ry12_order6)[1:length(V(p2ry12_order6))] %>% attr("name") %>% unique()


genes_on_order0 <- "P2ry12"
genes_on_order1 <- setdiff(genes_in_order1, genes_on_order0)
genes_on_order2 <- setdiff(genes_in_order2, genes_in_order1)
genes_on_order3 <- setdiff(genes_in_order3, genes_in_order2)
genes_on_order4 <- setdiff(genes_in_order4, genes_in_order3)
genes_on_order5 <- setdiff(genes_in_order5, genes_in_order4)
genes_on_order6 <- setdiff(genes_in_order6, genes_in_order5)
```
...
all_short_path <- shortest_paths(ig, V(ig)$name == "P2ry12", V(ig)$name %in% all_DEG)[[1]]

all_path_nodes <- sapply(all_short_path[-5], as_ids) %>% 
  t() %>% 
  rbind(c(all_short_path[[5]] %>% as_ids, NA)) %>% 
  as.data.frame() %>% 
  magrittr::set_colnames(c("order0", "order1", "order2", "order3"))

o1 <- all_path_df %>% 
  filter(from %in% (all_path_nodes %>% pull(order0) %>% unique()), to %in% (all_path_nodes %>% pull(order1) %>% unique()))

o2 <- all_path_df %>% 
  filter(from %in% (all_path_nodes %>% pull(order1) %>% unique()), to %in% (all_path_nodes %>% pull(order2) %>% unique()))

o3 <- all_path_df %>% 
  filter(from %in% (all_path_nodes %>% pull(order2) %>% unique()), to %in% (all_path_nodes %>% pull(order3) %>% unique))

filtered_path <- rbind(o1, o2, o3)

N <- tibble(
  name = c(filtered_path$from, filtered_path$to) %>% unique()
) %>% 
  mutate(shape = ifelse(grepl("^cpd:|^gl:|^dr:", name, ignore.case = F), "21", "22")) 

E <- filtered_path %>% 
  dplyr::select(from, to, from_entrez, to_entrez, everything())
```
...
g %>%  
  ...
  geom_edge_link(aes(filter = angle == 90, 
                 ...
                 end_cap = circle(3, "mm"), 
                 show.legend = F) +
  geom_node_point(aes(filter = !(name %in% all_DEG)), color ="grey", size = 5) +
  geom_node_point(aes(filter = (name %in% all_DEG)), color = "red", size = 5) +
  geom_node_text(aes(label = name), repl = T) 

ggsave(filename = "p2ry12_downstream_signal.tiff")
```
...
DEG_14_orders <- g %>% 
  ...
  filter(.N()$name %in% intersect(A14_DEG, L14_DEG)) %>% 
  as_tibble() %>% 
  pull(dfsr) %>% 
  table() %>% 
  as.data.frame() %>% 
...
DEG_3_orders <- g %>% 
  dplyr::mutate(dfsr = bfs_dist(root = .N()$name == "P2ry12", mode = "out")) %>% 
  filter(.N()$name %in% intersect(A3_DEG, L3_DEG)) %>% 
  as_tibble() %>% 
  pull(dfsr) %>% 
  table() %>% 
  as.data.frame() %>% 
...
DEG_3_orders %>% 
  ...
  scale_fill_brewer(name = "days", type = "qual", palette = "Set1", direction = -1) +

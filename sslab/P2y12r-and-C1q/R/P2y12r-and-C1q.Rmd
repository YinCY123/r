...
distance_to_p2ry12_all_path <- shortest_paths(ig, from = V(ig)$name == "P2ry12", to = V(ig), mode = "out")[[1]]
...
inter_len <- apply(all_path_nodes, 1, intersect, all_DEG) %>% sapply(length)
...
filtered_path <- rbind(o1, o2, o3, o4, o5, o6, o7)
filtered_path %>% pull(lty) 

N <- tibble::tibble(
  symbol = c(filtered_path$from, filtered_path$to) %>% unique(),
  ids = c(filtered_path$from_entrez, filtered_path$to_entrez) %>% unique()
) %>% 
...
ll %>%  
  ...
  geom_edge_link(aes(filter = (angle == 90), 
                 ...
                 show.legend = F, 
                 edge_width = 0.2) +
  geom_node_point(aes(shape = shape, filter = !(symbol %in% A3_L3_DEG_inter)), 
                  fill ="grey", 
                  size = 5, 
                  show.legend = F, 
                  color = "grey") +

...
# PVN top 100 GO DGE map to KEGG pathway  
## download mapped KEGG pathways  
```{r}
...
```{r}
parseKGML2Graph(file = "../res/P2ry12/A3_L3_top100_pathways/mmu00240.xml", genesOnly = F, expandGenes = T)
```


```{r}
parseKGML2Graph(file = "../res/P2ry12/A3_L3_top100_pathways/mmu00240.xml") %>% 
  getKEGGedgeData() %>% 
  .[[1]]
```
...
```{r}
KGML2DataFrame <- function(filepath, ...){
  # load required packages
  require(KEGGgraph, quietly = TRUE)
  require(magrittr, quietly = TRUE)
  
  parsedKGML <- parseKGML(file = filepath, ...)
  parsedGraph <- parseKGML2Graph(file = parsedKGML, genesOnly = F, expandGenes = T)
  graph_edge_list <- getKEGGedgeData(graph = parsedGraph)
  ids <- sapply(graph_edge_list, getEntryID)
  return(ids)
}

files <- list.files(path = "f:/git/R-codes/sslab/P2y12r-and-C1q/res/P2ry12/A3_L3_top100_pathways/", 
                    pattern = ".xml$", 
                    full.names = T)

KGML2DataFrame(filepath = "../res/P2ry12/A3_L3_top100_pathways/mmu00230.xml")
```


# map 3 days and 14 days  
```{r}
subtype_element <- data.frame(
  name = c("compound", "hidden compound", "activation", "inhibition", 
           "expression", "reexpression", "indirect effect", "state change", 
           "binding/association", "dissociation", "missing interaction", 
           "phosphorylation", "dephosphorylation", "glycosylation", "ubiquitination", 
           "methylation"), 
  value = c("Entry element id attribute value for compound", 
            "Entry element id attribute value for hidden compound", 
            "-->", "--|", "-->", "--|", "..>", 
            "...", "---", "-+-", "-/-",
            "+p", "-p", "+g", "+u", "+m")
)

...
```{r}
bg_genes <- filtered_pvn %>% pull(geneid_symbol)
bg_genes_df <- select(org.Mm.eg.db, 
                      keys = bg_genes, 
                      keytype = "SYMBOL", 
...
P2ry12_downstream_genes <- P2ry12_downstream_geens[grepl("^[0-9]", P2ry12_downstream_genes)]
...
pvn_A3_L3_DEG_GO %>% 
  ...
  ggplot(aes(p_value, reorder(Term, p_value, ), fill = group)) +
  geom_bar(stat = "identity", show.legend = F) +
  scale_y_discrete(name = NULL) +
  scale_x_continuous(name = "log10(p.value)", 
                     breaks = c(-10.0, -5.0, 0.0, 5.0, 10.0), 
                     labels = c("10.0", "5.0", "0.0", "5.0", "10.0")) +

...
write.csv(x = pvn_filtered, file = "../res/P2ry12/pvn_filtered.csv", row.names = F, quote = F)
```
...
```{r}
# common up and down genes
pvn3_common_up_genes <- xlsx::read.xlsx("../data/20201204_common/20201204_geneid_up_common_list.xlsx", 
                                        sheetName = "PVN_3") %>% 
  pull(up_PVN_3) %>% 
  as.character()

pvn3_common_down_genes <- xlsx::read.xlsx("../data/20201204_common/20201204_geneid_down_common_list.xlsx", 
                                          sheetName = "PVN_3") %>% 
  pull(down_PVN_3) %>% 
  as.character()
```
...
g %>%  
  ...
  geom_edge_link(aes(filter = (angle == 90), 
                 ...
                 show.legend = F, 
                 edge_width = 0.2) +
  geom_edge_link(aes(edge_linetype = factor(lty), filter = (is.na(angle))), 
  ...
  geom_node_point(aes(shape = shape), 
                  ...
                  show.legend = F, 
                  color = "grey") +
  geom_node_point(aes(shape = shape, filter = (names_to_display[.N()$ids] %in% pvn3_common_up_genes)), 
                 size = 5, 
                 color = "darkred", 
                 fill = "darkred") +
  geom_node_point(aes(shape = shape, filter = (names_to_display[.N()$ids] %in% pvn3_common_down_genes)),
                  size = 5,
                  color = "darkblue",
                  fill = "darkblue") +
...
ggsave(filename = "../res/P2ry12/p2ry12_downstream_signal-20210206.pdf")
```

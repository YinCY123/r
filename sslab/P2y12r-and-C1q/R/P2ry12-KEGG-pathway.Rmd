...
inter_len <- apply(all_path_nodes[1:10], 
                   1, 
                   intersect, 
                   mapIds(org.Mm.eg.db, keys = df$symbol %>% unique(), keytype = "SYMBOL", column = "ENTREZID")) %>%
  lapply(na.omit) %>% 
  sapply(length)
...
df$col <- ifelse(grepl("up", df$group), "darkred", "darkblue")
df_up <- df %>% filter(col == "darkred")
df_down <- df %>% filter(col == "darkblue")
...
g %>%  
  ...
  geom_node_point(aes(shape = shape), 
                  ...
                  show.legend = F, 
                  color = "grey") +
  geom_node_point(aes(shape = shape, filter = (names_to_display[.N()$ids] %in% df_up$symbol)), 
                 size = 5, 
                 color = "darkred", 
                 fill = "darkred") +
  # geom_node_point(aes(shape = shape, filter = (names_to_display[.N()$ids %in% df_down$symbol])), 
  #                 size = 5, 
  #                 color = "darkblue", 
  #                 fill = "darkblue") +
...
ggsave(filename = "../res/P2ry12/p2ry12_downstream_signal-20210116.pdf")
```

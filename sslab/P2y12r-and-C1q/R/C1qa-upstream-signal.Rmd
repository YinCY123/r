...
lty <- c("compound" = "blank", 
         "NA" = "blank", 
         "activation" = "solid", 
         "inhibition" = "solid", 
         "expression" = "dashed", 
         "binding/association" = "longdash", 
         "missing interaction" = "1F", 
...
all_path_df$lty <- ifelse(is.na(all_path_df$lty), "blank", all_path_df$lty)
all_path_df$edge_label <- ifelse(is.na(all_path_df$edge_label), "", all_path_df$edge_label)

all_path_df %>% pull(edge_label) %>% unique()
all_path_df %>% pull(lty) %>% unique()
```


```{r}
ig <- graph_from_data_frame(d = all_path_df[, c("from", "to")], directed = T)
distance_to_C3_all_path <- shortest_paths(ig, from = V(ig)$name == "12266", to = V(ig), mode = "in")[[1]]
...
```{r, message=FALSE, warning=FALSE}
g %>% 
  mutate(dist_to_C3 = bfs_dist(root = .N()$ids == "12266")) %>% 
  ...
  geom_node_point(aes(shape = shape), 
                  ...
                  fill = "grey", 
                  color = "grey") +
  geom_node_point(aes(filter = (ids %in% c("12266"))), 
                  fill = "red", 
                  color = "red", 
                  size = 2) +
  scale_shape_manual(values = c("21" = 22, "22" = 21))

ggsave(filename = "../res/C1q-C3/C3-signal-path_order4.pdf")
```

```{r}
filtered_path %>% pull(subtype) %>% unique()
```

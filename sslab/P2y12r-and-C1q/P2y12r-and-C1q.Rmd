...
Gnai1_order1 <- make_ego_graph(graph = all_path_graph, 
                               order = 1, 
                               nodes = "Gnai1", 
                               mode = "out")[[1]]

ll <- layout_as_star(graph = Gnai1_order1, center = "Gnai1")
ll %>% head()
ll %>% range()

vcol <- rep("gold", V(Gnai1_order1)$name %>% length())
vcol[V(Gnai1_order1)$name == "Gnai1"] <- "red"
vcol %>% table()
...
```{r}
Gnai2_order1 <- make_ego_graph(graph = all_path_graph, 
                               order = 1, 
                               mode = "out", 
                               nodes = "Gnai2")[[1]]

vcol <- rep("grey", V(Gnai2_order2)$name %>% length())
vcol[V(Gnai2_order2)$name == "Gnai2"] <- "red"
vcol %>% table()

ll <- layout_as_star(graph = Gnai2_order2, center = "Gnai2")
ll %>% head()
ll %>% range()

pdf(file = "Gnai2_order1.pdf")
plot(Gnai2_order2, 
     vertex.color = vcol, 
     vertex.frame.color = NA, 
     vertex.label.cex = 0.5, 
...
```{r}
Gnai3_order1 <- make_ego_graph(graph = all_path_graph, 
                               order = 1, 
                               mode = "out", 
                               nodes = "Gnai3")[[1]]

vcol <- rep("grey", V(Gnai3_order1)$name %>% length())
vcol[V(Gnai3_order1)$name == "Gnai3"] <- "red"
vcol %>% table()

ll <- layout_as_star(graph = Gnai3_order1, center = "Gnai3")
ll %>% head()
ll %>% range()

pdf(file = "Gnai3_order1.pdf")
plot(Gnai3_order1, 
     vertex.color = vcol, 
     vertex.frame.color = NA, 
     vertex.label.cex = 0.5, 
...
```{r}
nodes_Gnai1 <- V(Gnai1_order1)$name
nodes_Gnai2 <- V(Gnai2_order1)$name
nodes_Gnai3 <- V(Gnai3_order1)$name
```

...
```{r}
KGML_files <- list.files(path = "/home/yincy/git/Data/KEGG/pathways/mouse/", full.names = T, pattern = ".xml$")
...
all_path_df %>% head()
df <- df %>% distinct(from, to, .keep_all = T)
...
all_path_df <- all_path_df %>% 
    dplyr::select(from, to, from_symbol, to_symbol, type, subtype)
```
...
ll <- layout_with_lgl(graph = all_path_graph)
ll %>% head()
ll %>% range()
ll <- norm_coords(layout = ll, 
                  xmin = -w, xmax = w, 
                  ymin = -w, ymax = w)
```
...
```{r}
fw <- 10
pdf(file = "all_path.pdf", width = fw, height = fw)
plot(all_path_graph, 
     vertex.size = 10,
     layout = ll, 
     rescale = F, 
     ...
     edge.curved = 0.3, 
     edge.arrow.size = 0.5, 
     vertex.label = E(all_path_graph)$from_symbol)

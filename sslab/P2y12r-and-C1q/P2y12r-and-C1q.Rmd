...
```{r}
go_geneID <- str_split(string = go_list_pvn_2$geneID, pattern = "/")
go_list_pvn_2$overlap <- lapply(go_geneID, FUN = intersect, y = intersects) %>% sapply(str_c, collapse = "/")

write.xlsx2(x = go_list_pvn_2, 
            file = "GO list/pvn_uniq.xlsx", 
            sheetName = "co_down_GO", 
            append = T)
```
...
```{r}
pvn_2_3up_GO <- read.xlsx(file = "GO list/20200812_geneid_updown_go_list_PVN_2.xlsx", 
                          sheetName = "3up_GO") %>% 
    as_tibble() %>% 
    arrange(pvalue) %>% 
    head(n = 100) %>% 
...
pvn_2_3down_GO <- read.xlsx(file = "GO list/20200812_geneid_updown_go_list_PVN_2.xlsx", 
                          sheetName = "3down_GO") %>% 
    as_tibble() %>% 
    arrange(pvalue) %>% 
    head(n = 100) %>% 

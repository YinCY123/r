...
```{r}
x <- regexec("<dd>[F|f]ound on (.*?)</dd>", homicides)
x[1:3]
sapply(x, "[[", 1) %>% head

substr(homicides, 
       start = sapply(x, "[[", 1), 
       stop = c(sapply(x, "[[", 1) + sapply(x, attr, which = "match.length") %>% sapply("[[", 1)))
```

```{r}
sapply(x[1:3], attr, which = "match.length")
```
...
```{r}
x
substr(homicides, start = x, stop = (x + attr(x, "match.length")) - 1) %>% length()
```

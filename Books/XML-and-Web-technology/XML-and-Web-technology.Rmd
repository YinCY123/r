...
### Extracting an Entire Observation: A different Approach  
```{r}
doc <- xmlParse(file = "Data/Kiva/lenders/1.xml")
lenderNodes <- getNodeSet(doc = doc, path = "//lender")
varNames <- xmlApply(lenderNodes, names) %>% unlist() %>% table() %>% names()

obs <- lapply(X = getNodeSet(doc, "//lender"), 
              FUN = function(node){
                  ans = structure(sapply(node[varNames], xmlValue), names = varNames)
                  ans["loan_count"] = if(!is.null(a <- node[["loan_count"]])){
                      xmlValue(a)
                  }else{
                      NA
                  }
...
do.call(rbind, obs) %>% as.data.frame()%>% head 
```



```{r}
attrs <- c("name", "id", "type")

xmlAttrs_df <- function(file, path, attrs, default = -1){
    doc = xmlParse(file = file)
    ns <- getNodeSet(doc = doc, path = path)
    df <- data.frame(x = seq_along(ns))
    for(attr in attrs){
        x <- xpathSApply(doc = doc, path = path, fun = xmlGetAttr, name = attr, default = default)
        df <- cbind(df, attr = x)
    }
    colnames(df) <- c("x",attrs)
    return(df)
}

xmlAttrs_df(file = files[1], path = "//entry", attrs = attrs) 
```

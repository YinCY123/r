...
```{r}
doc <- htmlParse(file = "files/2020.html")
getNodeSet(doc, path = "//table")
readHTMLTable(doc = "files/2020.html", which = 2)
```


```{r}
doc <- htmlParse(file = "files/The New York Times - Breaking News, World News & Multimedia.html")
storyDivs <- getNodeSet(doc = doc, 
                        path = "//div[@class = 'css-142l3g4']")
...
```{r}
sapply(storyDivs, function(x) xmlName(x[[1]])) %>% table()
```

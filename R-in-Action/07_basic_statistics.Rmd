...
mytable <- xtabs(~ Sex + Improved, data = Arthritis)
fisher.test(mytable, alternative = "two.side")
```
...
- partial correlations: is a correlation between two quantitative variables, controlling for one or more other quantitative variables. `ggm` provides `pcor()` function to calculate partial correlation coefficients.  
...
> When the assumptions for the t-tests are reasonable, the parametric tests are more powerful (more likely to find a difference if it exists). The nonparametric tests are more appropriate when the assumptions are grossly unreasonable.  

...
Most of the functionality is `fs` can be approximated with functions in base R or in a command line shell.  

**fs** functions smooth over some of the idiosyncrasies of file handling with base R functions:  

- Vectorization. All `fs` functions are vectorized, accepting multiple path as input. Base functions are inconsistently vectorized.  

- Predictable return values that convey a path. All `fs` functions return a character vector of paths, a named integer or a logical vector, where the names give the paths. Base return values are more varied: they are often logical or contain error codes which requir downstream processing.   
...
- UTF-8 all the things. `fs` functions always convert input paths to UTF-8 and returns as UTF-8. This gives you path encoding consistency across OS. Base functions rely on the native system encoding.  

- Naming convention. `fs` functions use a consistent naming convention. Because base R's functions were gradually added over time there are a number of different conventions used (e.g. `path.expand()` VS `normalizedPath()`; `Sys.chomd()` VS `file.access()`).  
...
|`dir_info("path")`|do.call(rbind, <br> lapply(list.files("path"), <br> file.info))| ls -al path|
|`dir_copy("path", "new-path")`|`dir.reate("new-path")`; <br> `file.copy("path", "new-path", recursive = TRUE)`|cp path new-path|
|`dir_create("path")`|`dir.create("path")`|make path|
|`dir_delete("path")`|`unlink("path", recursive = TRUE)`|rm -rf path|
|`dir_exists("path")`|`dir.exists("path")`|if [-d "path"]; then ...; fi|
|(see `file_move`)|`file.rename("path", "new-path")`|mv path new-path|
...
|`path_real("path")`|`normalizePath()`|realpath|

...
```{r ligand-target-pairs, echo=FALSE, message=FALSE}
...
```{r load-single-cell-image, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r single-cell-plot, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r pseudotime-and-percent.mt, message=FALSE, warning=FALSE}
plot_grid(pseudotime_p, 
          percent.mt_p)
```

```{r expression-of-ligands, echo=FALSE, message=FALSE, warning=F}
...
```{r expression-of-target, echo=FALSE, message=FALSE, warning=FALSE}
...
### Discussion  
neovascularization is the form of late stage AMD, althougth anti-VEGF has revolutionized the treatment of wet AMD, but some drawbacks have been reported, such as the therapy is not durable even after intensive 2 years treatment and most of the visual gain was lost[@Cheung2017;@CATT_Research_Group2016] after the cease of therapy. Genome-Wide Association Studies (GWAS) have identified dozens of sites associated with AMD. which implies immune system, 
...
### Author Contribution  
Chunyou conceive, design and conducted the data analysis, Chunhui help build the computational environment and some help on dimention reduction algorithm, Xiao write the manuscripte.  
...
```{r statistic-result-ligands, echo=FALSE}
